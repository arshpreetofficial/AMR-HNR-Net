...
def train_cnad(fold, model, opt, train_loader, test_loader, epochs, optimizer, scheduler, criterion,
    ...
    """
     Executes the training and validation loop for the single-modal network.

    """
    steps = 0
    start = time.time()

    #  Initialize best metric tracker OUTSIDE the epoch loop
    best_auc = 0.0

    for e in tqdm(range(1, epochs + 1), desc=f"Fold {fold} Training"):
        model.train()

        # Clean slate for gradient accumulation at epoch start
        optimizer.zero_grad()
        ...
        for ii, item in enumerate(tqdm(train_loader, desc=f"Epoch {e}/{epochs}", leave=False)):
            ...
            # Forward pass
            outputs, _, expert_outputs, shared_outputs, expert_inputs, recon_outputs, _ = model.forward(images, atlases)
            loss_cls = criterion(outputs, labels)

            # Atlas methodology loss
            ...
            # Backward pass & step
            loss_scaler(loss, optimizer, parameters=model.parameters(), update_grad=(ii + 1) % accum_iter == 0)

            if (ii + 1) % accum_iter == 0:
                optimizer.zero_grad()

            train_loss += loss.item() * accum_iter  # Rescale for accurate logging
            ...
            train_label_all.extend(labels.cpu().numpy())  # [FIX] Convert to numpy to prevent leak
        ...
        with torch.no_grad():
            for ii, item in enumerate(test_loader):
                ...
                outputs, _, expert_outputs, shared_outputs, expert_inputs, recon_outputs, _ = model(images, atlases)
                ...
                val_label_all.extend(labels.cpu().numpy())  # [FIX] Convert to numpy
